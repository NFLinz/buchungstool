...
@Component({
  selector: 'app-admin-company-booking',
  templateUrl: './admin-company-booking.component.html',
  styleUrls: ['./admin-company-booking.component.css']
})
export class AdminCompanyBookingComponent implements OnInit {
  ...
  calculatedAmountNonMemberAdults = 0;         // real calculated amount (no of non-members who are adults at the day of arrival)
  ...
  constructor(
      ...
      this.create_booking_form = this.formBuilder.group({
          ...
          // necessary controls for booking storage:
          ...
          erwachsene: this.calculatedAmountNonMemberAdults,
          jugendliche: this.calculatedAmountNonMemberTeenagers,
          kinder: this.calculatedAmountNonMemberKids,
  ...
  verpflegungSelectOnChange(_value: string) {
    ...
    adultMemberSelectOnChange(_value: number) {
        // console.log("select: "+_value)
        this.calculatedAmountMemberAdults = _value;
    }

    adultNonMemberSelectOnChange(_value: number) {
        // console.log("select: "+_value)
        this.calculatedAmountNonMemberAdults = _value;
    }

    teenagerMemberSelectOnChange(_value: number) {
        // console.log("select: "+_value)
        this.calculatedAmountMemberTeenagers = _value;
    }

    teenagerNonMemberSelectOnChange(_value: number) {
        // console.log("select: "+_value)
        this.calculatedAmountNonMemberTeenagers = _value;
    }

    kidMemberSelectOnChange(_value: number) {
        // console.log("select: "+_value)
        this.calculatedAmountMemberKids = _value;
    }

    kidNonMemberSelectOnChange(_value: number) {
        // console.log("select: "+_value)
        this.calculatedAmountNonMemberKids = _value;
    }

    calcTotalPersons() {
        return  Number(this.calculatedAmountMemberAdults) + Number(this.calculatedAmountNonMemberAdults) + 
                Number(this.calculatedAmountMemberTeenagers) + Number(this.calculatedAmountNonMemberTeenagers) + 
                Number(this.calculatedAmountMemberKids) + Number(this.calculatedAmountNonMemberKids);
    }
  ...
  validateForm() {
    ...
    switch (this.currentTab) {
        case 0:    // tab 1
            //   console.log("validating tab: " + this.currentTab)
        ...
        case 2:   // booking person
            //   console.log("validating tab: " + this.currentTab)
              ...
              if (this.create_booking_form.get('anrede').value === '') {
                isFormValid = false;
              }

              var inputFields = elem.getElementsByTagName("input");
              for (var i = 0; i < inputFields.length - 1; i++) {    // check every inputfield in the current tab except for 'bemerkung'
                  // console.log("inputfield " + i + ": " + inputFields[i].value)
          ...
          case 4: // payment method
            //   console.log("validating tab: " + this.currentTab)
              ...
              break;
        default:
            //   console.log("validation default switch")
            isFormValid = false;
            break;
    }
    // console.log("isvalid after switch: " + isFormValid)
